...
| 영역 | 무엇을, 어떻게 |
|---|---|
| **이미지 레지스트리 네임스페이스 통일** | ~~각 서비스 차트의 `helm/<service>/values.yaml`·`values.amd64.yaml`이 보는 이미지가 `khg1115/groovy-*`(수동 빌드 스냅샷)인데, poly 서비스 레포들의 (현재 비활성화된) CI는 `bebeghi/groovy-*`로 push하도록 되어 있음. 둘 중 하나로 정하고 `values*.yaml`의 `repository` 필드를 그 값으로 갱신해야 함.~~ **(GitOps 빌드 지시서 Phase 06 완료: 전 차트를 `ttt103/groovy-*`로 통일. api-gateway만 예외로 Docker Hub 리포지토리명이 `ttt103/groovy-gateway-service`.)** |
...
| **서비스 CI 활성화** | ~~poly 7개 레포(`groovy-frontend` + 6개 서비스)의 `.github/workflows/docker-build-push.yml.disabled`를 활성화하고, 각 GitHub 레포에 `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` 시크릿을 새로 등록해야 함(모놀리식 레포 시크릿은 자동 상속 안 됨).~~ **(GitOps 빌드 지시서 Phase 06 완료: 7개 레포 모두 `.github/workflows/build-and-deploy.yml`로 활성화됨. `@SpringBootTest` 통합 테스트 CI 편입은 각 서비스 워크플로 상단 TODO로 남아있는 채 보류.)** |
| **ArgoCD 기반 CD 신설** | ~~지금 이 레포엔 ArgoCD `Application`/`AppProject` 매니페스트가 전혀 없음. 서비스 CI가 이미지를 푸시하면 이 레포의 이미지 태그를 갱신하고, ArgoCD가 그 변경을 감지해 클러스터에 동기화하는 GitOps 파이프라인(계획서 5장)을 이 레포 안에 새로 구성해야 함.~~ **(GitOps 빌드 지시서 Phase 04/06 완료: `argocd/root-app.yaml` + `argocd/apps/*.yaml` 9개로 App of Apps 구성, 서비스 CI의 마지막 스텝이 이 레포의 `values.yaml` `tag:`를 갱신·커밋해 ArgoCD sync를 트리거함. 단, `values-secret.yaml` 최초 주입과 실제 ArgoCD 설치·클러스터 검증은 아직 미완.)** |
